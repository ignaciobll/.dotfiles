...
;;
;; Label names and cite names are found from \label{} and \bibitem{} in the
...
;; `tex-mode' by default.  M-x auto-complete-mode can try it temporarily.
...
;; Version 3 - new email
;; Version 4 - allow for duplicate labels
...
(eval-when-compile
  (unless (fboundp 'push)
    (require 'cl))) ;; for push macro, maybe
...
\\\\label{\\([^}\r\n]+\\)"
        ...
        (cond ((match-beginning 1)
              ...
              (t
               ;; \label{name}
               ;; all label names should be unique, but check just in case
               (unless (member (match-string 2) ret)
                 (push (match-string 2) ret))))))
    (nreverse ret)))
...
\\\\bibitem{\\([^}\r\n]+\\)"
        ...
        (cond ((match-beginning 1)
              ...
              (t
               ;; \bibitem{name}
               ;; all bibitem names should be unique, but check just in case
               (unless (member (match-string 2) ret)
                 (push (match-string 2) ret))))))
    (nreverse ret)))
